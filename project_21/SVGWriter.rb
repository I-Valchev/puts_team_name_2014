...
class SVGWriter
	...
	def write data, time_taken
		...
		0.upto(@checker.HOMEWORK_NAMES.size) do |init_ind|
			valid_tasks[init_ind] = 0
		end

		data.each do |value, i|
			0.upto(@checker.HOMEWORK_NAMES.size){|checked_index|
				valid_tasks[checked_index] += 1 if i[checked_index].to_i == 2
			}
		end
		num_hws = 0;
		avr_flay = 0;
		flay_start = @checker.HOMEWORK_NAMES.size + 1
		flay_start.upto(flay_start + @checker.HOMEWORK_NUMBERS_FLOG_FLAY.size-1) do |flay_val|
		 	data.each do |student|
		 		num_hws+=1
		 		avr_flay += student[1][flay_val].to_s.gsub(/[^\d]/,"").to_f unless student[1][flay_val].is_a?(Numeric)
		 	end
		end 
		avr_flay/=num_hws
		avr_flog = 0
		num_hws = 0
		flog_start = flay_start + @checker.HOMEWORK_NUMBERS_FLOG_FLAY.size
		flog_start.upto((flog_start + @checker.HOMEWORK_NUMBERS_FLOG_FLAY.size)-1) do |flog_val|
			data.each do |student|
				num_hws+=1
		  		avr_flog += student[1][flog_val].to_s.gsub(/[^\d]/,"").to_f unless student[1][flog_val].is_a?(Numeric)
		 	end
		end
		avr_flog/=num_hws
		b = Nokogiri::XML::Builder.new do |doc|
		  doc.svg xmlns:"http://www.w3.org/2000/svg", viewBox:"0 -400 500 500", style:"background: #FFFFCC" do
		  	...
		  	@checker.HOMEWORK_NAMES.each_with_index do |hw_name, column|
